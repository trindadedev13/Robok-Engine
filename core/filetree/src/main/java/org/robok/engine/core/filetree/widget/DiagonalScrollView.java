...
/*
 * Based on AOSP's HorizontalScrollView, here it's Copyright:
 *
 ...
 * than the physical display. A DiagonalScrollView is a {@link FrameLayout}, meaning you should
 * place one child in it containing the entire contents to scroll; this child may itself be a layout
 * manager with a complex hierarchy of objects. A child that is often used is a {@link
 * android.widget.TableLayout}.
 *
 * <p>You should never use a DiagonalScrollView with a {@link android.widget.ListView}, because
 * ListView takes care of its own scrolling. Most importantly, doing this defeats all of the
 * important optimizations in ListView for dealing with large lists, since it effectively forces the
 * ListView to display its entire list of items to fill up the infinite container supplied by
 * DiagonalScrollView.
 */
public class DiagonalScrollView extends FrameLayout {
  ...
  /**
   * True if the user is currently dragging this ScrollView around. This is not the same as 'is
  ...
  public DiagonalScrollView(Context context, AttributeSet attrs, int defStyle) {
  ...
  /**
   * Indicates this ScrollView whether it should stretch its content height to fill the viewport or
   * not.
   *
    ...
    switch (action & MotionEvent.ACTION_MASK) {
      ...
      case MotionEvent.ACTION_UP:
        if (mIsBeingDragged) {
          ...
          if (getChildCount() > 0) {
            if (((Math.abs(initialVelocityX) + Math.abs(initialVelocityY)) / 2
                > mMinimumVelocity)) {
        ...
        case MotionEvent.ACTION_SCROLL:
          {
            if (!mIsBeingDragged) {
              ...
              if (hscroll != 0 || vscroll != 0) {
                final int deltaX = (int) (hscroll * MAX_SCROLL_FACTOR);
                final int rangeX = getScrollRangeX();
                final int deltaY = (int) (vscroll * MAX_SCROLL_FACTOR);
                final int rangeY = getScrollRangeY();
  ...
  private int getScrollRangeY() {
    ...
    if (getChildCount() > 0) {
      ...
      scrollRange =
          Math.max(0, child.getHeight() - (getHeight() - getPaddingBottom() - getPaddingTop()));
    }
    return scrollRange;
  }
  ...
  private int getScrollRangeX() {
    ...
    if (getChildCount() > 0) {
      View child = getChildAt(0);
      scrollRange =
          Math.max(0, child.getWidth() - (getWidth() - getPaddingLeft() - getPaddingRight()));
    }
    return scrollRange;
  }
  ...
  private boolean isWithinDeltaOfScreen(View descendant, int width, int height) {
    ...
    return mTempRect.bottom >= getScrollY()
        && mTempRect.top <= (getScrollY() + height)
        && mTempRect.right >= getScrollX()
  ...
  /**
   * Compute the amount to scroll in the X direction in order to get a rectangle completely on the
   * screen (or, if taller than the screen, at least the first screen size chunk of it).
   *
   * @param rect The rect.
   * @return The scroll delta.
   */
  private int computeScrollDeltaXToGetChildRectOnScreen(Rect rect) {
   ...
   * @return The scroll delta.
   */
  private int computeScrollDeltaYToGetChildRectOnScreen(Rect rect) {
    if (getChildCount() == 0) return 0;
   ...
   *     finger/cursor is moving right the screen, which means we want to scroll towards the left.
   * @param velocityY The initial velocity in the Y direction. Positive numbers mean that the
   *     finger/cursor is moving down the screen, which means we want to scroll towards the top.
   */
  ...
  static class SavedState extends BaseSavedState {
    ...
    public String toString() {
      return "HorizontalScrollView.SavedState{"
          ...
          + " scrollXPosition="
          + scrollXOffsetFromStart
          + " scrollYPosition="
          + scrollYOffsetFromStart
          + "}";
    }
  }
}
